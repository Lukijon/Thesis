"""Extracts raw text from every acquired Risk Factors PDF
(src/acquisition/cvm_fre.py) and computes year-over-year TF-IDF cosine
similarity -- same measure, same corpus-wide IDF fit, applied to the
theoretically strongest text source tried this session (see
reports/return_predictability_exploration.md and
reports/panel_model_results.md for why: Cohen-Malloy-Nguyen's own effect
concentrates in Risk Factors more than any other section they test).
Mirrors src/processing/build_mgmt_report_corpus.py's structure exactly.

Usage:
    python -m src.processing.build_risk_factors_corpus
"""
...
RAW_ROOT = Path("data/raw/risk_factors")
LINES_CACHE = Path("data/interim/poc/risk_factors_lines_cache")
POC_INTERIM = Path("data/interim/poc")
RESULTS_CSV = POC_INTERIM / "risk_factors_similarity_results.csv"


def _corpus_pdfs() -> list[tuple[str, int, int, Path]]:
    items = []
    for pdf_path in sorted(RAW_ROOT.glob("*/*/fatores_risco.pdf")):
...
def main() -> None:
    ...
    print(f"{n_found}/{n_total} risk-factors filings with substantive text (>100 chars)")
