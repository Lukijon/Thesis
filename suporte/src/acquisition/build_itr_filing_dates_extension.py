"""ITR filing-dates counterpart to build_filing_dates_extension.py: builds
the DT_RECEB reference table for the round-7 expanded universe's quarterly
acquisition (run_itr_extension.py, data/interim/itr_extension_log.csv,
6,260 company-quarters, 2011-2025). Unlike the annual case, this single
log is already a standalone superset of the original 112-company/
2015-2024 ITR panel -- full_universe_cd_cvm() in run_itr_extension.py
covers all 185 companies across all years, not just the incremental ones
-- so no merge with itr_filing_dates.csv is needed.

Usage:
    python -u -m src.acquisition.build_itr_filing_dates_extension
"""
...
CACHE_DIR = Path("data/raw/dfp/_cache")  # shared cache dir with DFP/ITR acquisition
INTERIM = Path("data/interim")
YEARS = range(2011, 2026)
OUT_PATH = INTERIM / "itr_filing_dates_extension.csv"
...
def main() -> None:
    ...
    log = pd.read_csv(INTERIM / "itr_extension_log.csv")
