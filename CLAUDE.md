...
- **Market data: received and verified.** `data/raw/market/prices/stock_prices_bloomberg.csv` (153 tickers, daily `PX_LAST`, 2014–2026) + `ibov_index_bloomberg.csv` (Ibovespa daily level, same period) — all 66 current-universe tickers covered; 9 tickers are stale pre-rename duplicates that resolve `#N/A` (e.g. EMBR3→EMBJ3, MRFG3→MBRF3, CCRO3→MOTV3) but the current-name column has full data, so use the current B3 ticker per company when mapping to CD_CVM. See `TODO.md` for the full verification notes. Not yet cross-checked: whether all 45 historical (non-current) companies are covered.
