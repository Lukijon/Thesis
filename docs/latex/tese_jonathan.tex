...
Empréstimos, financiamentos e debêntures são, para a maioria das empresas não financeiras, o item mais sensível a risco de todo o balanço: é onde vencimento, taxa, garantia, covenant e renegociação -- os termos que determinam se uma empresa consegue honrar ou rolar sua dívida -- estão descritos. O saldo agregado da dívida, sozinho, não carrega essa informação; ela só aparece no texto da nota explicativa. Se o mercado não lê essa nota com a mesma atenção com que lê o lucro ou o EBITDA do trimestre -- e há uma razão concreta para não ler: a nota é longa, técnica, e disputa atenção com dezenas de outras informações divulgadas no mesmo pacote --, uma mudança relevante no texto pode continuar fora do preço por algum tempo depois da divulgação. Esta dissertação testa se isso acontece no mercado brasileiro.

A pergunta, especificamente: empresas cuja nota de empréstimos, financiamentos e debêntures muda mais, de um exercício para o outro, têm retorno anormal diferente nos doze meses seguintes à divulgação, mesmo depois de controlar pelas mudanças observáveis nos fundamentos da empresa? A mudança textual é medida por TF-IDF e similaridade de cosseno entre o texto da nota em dois exercícios consecutivos -- a mesma lógica de \citeonline{brown2011} e de \citeonline{cohen2020} (``Lazy Prices''), que encontram, para o mercado americano, que empresas cujos documentos regulatórios mudam mais no texto têm desempenho futuro pior, sem reação imediata de preço na data de divulgação. O recorte para a nota de dívida especificamente, e não o documento inteiro, seguindo antes \citeonline{amelzadeh2016} do que \citeonline{cohen2020} nesse ponto, é a escolha estrutural desta dissertação: \citeonline{amelzadeh2016} mostram que a reação do mercado é mais rápida ao MD\&A do que às notas explicativas de um 10-K americano, o que torna a nota -- e não o relatório da administração, mais lido e mais discutido -- o lugar mais plausível para encontrar sub-reação. É também uma pergunta ainda não respondida no Brasil: a literatura nacional sobre notas explicativas cobre legibilidade \cite{borges,silva2019}, tom \cite{muniz2025}, covenants \cite{schiozer2021} e, mais recentemente, um índice de informatividade construído com modelo de linguagem \cite{costaneto2025}, mas nenhum desses trabalhos testa se a mudança textual ano a ano, medida de forma simples e replicável, prevê retorno futuro.

O resultado principal: sobre uma amostra de 185 empresas não financeiras listadas na B3 e 1.134 pares empresa-ano (2010--2025), a mudança textual da nota de dívida anual está associada ao retorno anormal futuro de forma estatisticamente significativa a 5\% em toda a grade de controles testada, do diagnóstico simples ($p=0{,}010$) até a especificação com controles completos ($p=0{,}011$), passando pela especificação principal pré-especificada ($\beta=0{,}156$, $p=0{,}017$). O coeficiente sobrevive a exclusão de escores de similaridade degenerados, ao BHAR sem winsorização e à exclusão de cada empresa individualmente (Seção~\ref{sec:robustez}) -- não é um artefato de uma especificação isolada ou de uma empresa influente. Mas o resultado vem com uma qualificação que esta dissertação não suaviza: é o único, entre cinco fontes de texto testadas (a própria nota em base trimestral, o conjunto completo de notas explicativas, o Relatório da Administração, os Fatores de Risco do Formulário de Referência) e nove testes de especificação principal, a atingir significância a 5\%, e não sobrevive à correção formal para múltiplos testes (Benjamini-Hochberg, $p$ ajustado $=0{,}156$). A leitura adotada aqui, e sustentada ao longo dos Capítulos~\ref{ch:resultados}--\ref{ch:conclusao}, é a mais conservadora possível diante desse conjunto de evidências: um resultado candidato, robusto a checagens de especificação usuais, mas não confirmado sob controle formal do risco de falsa descoberta.

Essa especificidade -- o efeito aparece na nota de dívida, o texto mais formulaico e auditado dos cinco testados, e não no Relatório da Administração nem nos Fatores de Risco, as fontes de maior discricionariedade narrativa -- é também onde esta dissertação mais diverge da literatura internacional que a motiva. \citeonline{cohen2020} e \citeonline{amelzadeh2016} encontram o efeito concentrado exatamente nos textos de maior discricionariedade gerencial, o oposto do padrão aqui. Essa divergência, não a significância em si, é o achado mais informativo deste trabalho sobre como o mecanismo de sub-reação a texto pode operar de forma diferente no mercado brasileiro -- e é retomada com mais profundidade no Capítulo~\ref{ch:conclusao}.

A contribuição desta dissertação, portanto, tem três partes. Primeiro, ela testa no Brasil uma pergunta já respondida para o mercado americano por \citeonline{cohen2020} e \citeonline{brown2011}, mas nunca replicada aqui com o recorte específico de notas de dívida -- a literatura brasileira sobre notas explicativas, revisada no Capítulo~\ref{ch:referencial}, chega perto (legibilidade, tom, covenants) sem fazer esse teste. Segundo, ela usa como variável dependente um modelo de quatro fatores com alfa próprio por empresa (Seção~\ref{sec:var_dependente}), mais alinhado à tradição de \citeonline{barberlyon1997} para retorno anormal de longo prazo do que o retorno simplesmente ajustado ao índice usado em versões exploratórias anteriores deste projeto, e testa a especificação sob uma grade de controles pré-especificada (M0-M3, Seção~\ref{sec:controles}) antes de olhar para qualquer resultado. Terceiro, e mais diretamente ligado ao resultado acima, ela mostra que o efeito -- quando aparece -- aparece justamente onde a literatura internacional previa que fosse mais fraco, uma pista concreta de que replicar um método em um mercado diferente pode não replicar o padrão de onde o método funciona melhor, não apenas se o método funciona ou não.
...
O Capítulo~\ref{ch:referencial} situa esta pergunta na literatura sobre assimetria informacional, atenção limitada e mudança textual como sinal de informação nova, e detalha onde esta dissertação se encaixa e diverge dos trabalhos citados nesta introdução. O Capítulo~\ref{ch:metodologia} descreve a coleta e extração das notas, a medida de mudança textual, a variável dependente (BHAR ajustado sobre um modelo de quatro fatores), a grade de controles M0-M3 e a estratégia de identificação, incluindo as três ameaças à inferência endereçadas (observações repetidas, variável omitida no nível da empresa, causalidade reversa) e como cada uma é tratada. O Capítulo~\ref{ch:dados} descreve a amostra final, as cinco fontes de texto e três fontes de dados estruturados combinadas, e a confiabilidade da extração. O Capítulo~\ref{ch:resultados} reporta os resultados de H1 e H2 para cada fonte de texto, a bateria de robustez do resultado principal, e a correção para múltiplos testes. O Capítulo~\ref{ch:conclusao} discute as leituras alternativas para o padrão de resultados encontrado, suas limitações, e os próximos passos identificados ao longo do trabalho.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}llrr@{}}
        ...
        \midrule
        Nota de dívida isolada (total)      & Anual & 1.858 & 177 \\
        \rowcolor{fontechave}
        \textbf{Nota de dívida isolada (confiável)}  & \textbf{Anual} & \textbf{1.426} & \textbf{166} \\
        Nota de dívida isolada (total)      & Trimestral & 5.464 & 177 \\
        Nota de dívida isolada (confiável)  & Trimestral & 3.725 & 170 \\
        Conjunto completo de notas explicativas & Anual & 1.933 & 178 \\
        Relatório da Administração & Anual & 1.340 & 160 \\
        Fatores de Risco (FRE) & Anual & 1.964 & 179 \\
        \bottomrule
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Fonte: elaborada pelo autor a partir de dados da CVM. ``Total'' = todos os pares com similaridade computável; ``confiável'' = restrito a pares com extração confiável (\textit{font\_heading}) nos dois anos (Seção~\ref{sec:confiabilidade}), a amostra usada nos resultados a partir daqui. A linha em destaque (nota de dívida isolada, confiável, anual) é a fonte de texto principal desta dissertação (Seção~\ref{sec:medida}); as demais são comparações de robustez e generalização (Seção~\ref{sec:identificacao}).}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrrrr@{}}
        ...
        Nota de dívida (trimestral)  & 1.176 & 0,397 & 0,302 & \cellcolor{m2principal}0,269 & 0,334 \\
...
Sobre o universo expandido, a nota de dívida anual -- a fonte de texto principal desta dissertação -- é a única a mostrar associação estatisticamente significativa entre TextChange e o retorno anormal, e o é em todo estágio da grade M0-M3, inclusive na especificação principal (M2: $\beta=0{,}156$, $p=0{,}017$) e na robustez econômica completa (M3: $p=0{,}011$). Nenhuma das outras quatro fontes mostra associação significativa em nenhum estágio, o mesmo padrão nulo já documentado na versão original da amostra (111 empresas, 2015--2024). Essa especificidade -- o efeito aparece exatamente na fonte teoricamente mais motivada (Seção~\ref{sec:medida}) e em nenhuma das quatro usadas como comparação metodológica -- é discutida com mais detalhe adiante nesta seção, depois do detalhamento de coeficientes a seguir, e é submetida à bateria de robustez completa na Seção~\ref{sec:robustez}. A Tabela~\ref{tab:h1_m2_detalhe} detalha o coeficiente, o erro-padrão e o intervalo de confiança de 95\% de cada modelo e cada fonte.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}llrrrr@{}}
        ...
        Nota de dívida (trimestral) & M0 & 1.245 & $-$0,019 (0,023) & [$-$0,063; 0,025] & 0,397 \\
                                     & M1 & 1.230 & $-$0,023 (0,022) & [$-$0,066; 0,021] & 0,302 \\
                                     & M2* & 1.230 & $-$0,025 (0,022) & [$-$0,068; 0,019] & 0,269 \\
                                     & M3 & 1.176 & $-$0,022 (0,023) & [$-$0,067; 0,023] & 0,334 \\
    ...
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Fonte: elaborada pelo autor. EP = erro-padrão agrupado por empresa. *M2 é a especificação principal. Nota de dívida (anual e trimestral): amostra restrita a pares com extração confiável (Seção~\ref{sec:confiabilidade}); a nota anual é o único bloco de linhas cujo intervalo de confiança não contém zero em nenhum modelo da grade.}
\end{table}
...
O padrão de resultados por fonte de texto, incluindo essa especificidade, também não é aleatório em relação à teoria. \citeonline{cohen2020} e \citeonline{amelzadeh2016} documentam, dentro de suas próprias amostras, que o efeito de mudança textual sobre retorno futuro se concentra em texto de alta discricionariedade gerencial (Risk Factors, MD\&A) e é mais fraco em texto formulaico e auditado, como notas explicativas contábeis -- o oposto exato do padrão de significância encontrado aqui, em que a nota de dívida (o texto mais formulaico e auditado entre os cinco testados) é a única fonte significativa, e o Relatório da Administração e os Fatores de Risco (as fontes de maior discricionariedade narrativa, adquiridas especificamente para testar essa previsão da literatura) permanecem nulos em todo M0-M3. Essa discrepância com a literatura internacional é retomada com mais profundidade no Capítulo~\ref{ch:conclusao}; ela não invalida o resultado -- a robustez do coeficiente na fonte principal é testada extensivamente na Seção~\ref{sec:robustez} --, mas pesa contra uma leitura de que o mecanismo em jogo no mercado brasileiro seja idêntico ao documentado para o mercado americano.

O resultado da nota de dívida anual é lido com o mesmo cuidado que se aplicaria a um resultado nulo: um único coeficiente significativo, numa família de nove testes M2 (cinco fontes de H1 mais quatro de H2), é exatamente o que se esperaria sob a hipótese nula se nada real estivesse acontecendo -- a correção de múltiplos testes ao final deste capítulo trata esse ponto diretamente. O resultado nulo das outras quatro fontes, por sua vez, admite mais de uma explicação, e esta dissertação não escolhe uma única entre elas sem evidência adicional: (i) o mercado brasileiro pode incorporar a informação dessas divulgações rapidamente, antes da janela medida em cada caso; (ii) a similaridade de cosseno sobre TF-IDF capta mudança lexical, não necessariamente novidade econômica; (iii) informação relevante já pode estar refletida em outras divulgações que chegam ao mercado antes (fatos relevantes, teleconferências, releases de resultado); e (iv) o efeito verdadeiro pode ser pequeno e o poder estatístico insuficiente para distingui-lo de zero nessas quatro fontes especificamente. Essas leituras são retomadas com mais profundidade no Capítulo~\ref{ch:conclusao}.
...
\end{itemize}
Os três testes usam a mesma especificação M2 e o mesmo painel de efeitos fixos de empresa e ano com erros-padrão agrupados por empresa da Tabela~\ref{tab:h1}. O resultado sobrevive a todos, um padrão bem mais robusto do que os achados frágeis documentados nas subseções seguintes desta mesma seção (Relatório da Administração sob o desenho antigo; Fatores de Risco e deslistamento) -- mas, precisamente por isso, também sujeito a um escrutínio maior antes de ser tratado como conclusivo: as subseções de portfólio calendário, deslistamento e comparação com \citeonline{santoscoelho2018} a seguir ainda refletem o universo original de 111 empresas (2015--2024), não o universo expandido, e sua extensão ao universo de 185 empresas permanece como próximo passo (Capítulo~\ref{ch:conclusao}) -- a leitura completa deste resultado deve considerar essa lacuna.
...
\textbf{Teste de portfólio calendário para a nota de dívida (universo original, 111 empresas).} Este teste, e os dois seguintes (checagem de deslistamento, comparação com \citeonline{santoscoelho2018}), ainda não foram reestimados sobre o universo expandido de 185 empresas -- ver a nota ao final da bateria de robustez acima. Como verificação independente da ameaça de observações repetidas (Seção~\ref{sec:identificacao}), aplicou-se à nota de dívida anual -- a fonte de texto principal -- o teste de portfólio calendário longo-curto: carteiras formadas mensalmente por similaridade (terços, mantidas por 12 meses, para casar com o horizonte do BHAR ajustado) têm seu retorno testado pelo alfa de um modelo de quatro fatores idêntico ao usado para construir a variável dependente (Seção~\ref{sec:var_dependente}). O alfa anualizado é de $-8{,}9\%$ ($t=-1{,}89$, $p=0{,}058$): abaixo do limiar de 5\%, não replicado nas outras cinco combinações de janela (3 e 6 meses) e número de grupos (2 e 3) testadas ($p \geq 0{,}10$ em todas), e ainda mais fraco ($p=0{,}121$) ao excluir os 19 pares com similaridade degenerada (Seção~\ref{sec:poc_validacao}). Corrobora o resultado nulo da grade M0-M3 para a nota de dívida por um método independente.
...
\textbf{Correção para múltiplos testes.} A família confirmatória desta dissertação -- os nove testes da especificação principal (M2), cinco de H1 (Tabela~\ref{tab:h1}) e quatro de H2 (Tabela~\ref{tab:h2}, que permanece no universo original de 111 empresas) -- foi submetida à correção de Benjamini-Hochberg \cite{benjaminihochberg1995} (taxa de descoberta falsa, $q=0{,}05$). M0, M1 e M3 não entram nessa família por serem variantes cumulativas de robustez do mesmo coeficiente por fonte, não hipóteses independentes; os testes exploratórios e de robustez (deslistamento, comparação com o desenho de Santos e Coelho, portfólio calendário, bateria secundária de H2) também ficam de fora, pela mesma lógica de antes.

O menor p-valor bruto da família passa de $0{,}268$ (nota de dívida anual, H1, universo original) para $0{,}017$ no universo expandido -- mas, mesmo essa melhora considerável, \textbf{não é suficiente para sobreviver à correção}: o p-valor ajustado é $0{,}156$, acima do corte de 5\%. Nenhum dos nove testes sobrevive. Vale registrar com precisão o que esse número diz e o que não diz: o resultado da nota de dívida anual é forte o bastante para não ser descartado como artefato de uma única especificação -- sobrevive à bateria de robustez inteira reportada acima, coeficiente por coeficiente -- mas não é forte o bastante para ser tratado como uma descoberta isolada do resto do exercício exploratório mais amplo desta dissertação (cinco fontes de texto, duas hipóteses, múltiplos horizontes). A leitura adotada aqui é a mais conservadora possível diante desse conjunto: um resultado candidato, robusto às checagens de especificação usual mas não confirmado sob controle formal do risco de falsa descoberta -- nem uma coincidência a ser descartada, nem uma confirmação de H1 a ser proclamada. Essa correção deve ser refeita, no limite, se H2 for eventualmente estendida ao universo de 185 empresas.
