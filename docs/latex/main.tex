...
\noindent
\textbf{Palavras-chave:} notas explicativas; análise textual; TF-IDF; retorno anormal; disclosure; consenso de estimativas.
\thispagestyle{empty}
\newpage
...
O retorno anormal é calculado como o retorno \textit{buy-and-hold} da ação nos 12 meses seguintes à data de divulgação da DFP (data real de recebimento pela CVM, não a data de fechamento do exercício), menos o retorno \textit{buy-and-hold} do Ibovespa no mesmo período. Trata-se de um retorno anormal ajustado ao mercado (\textit{market-adjusted}), sem estimação de beta, na tradição metodológica descrita por \citeonline{barberlyon1997} para estudos de retorno de longo prazo. É também a mesma família de medida usada por \citeonline{brown2011} para o retorno em torno do arquivamento do 10-K, ainda que em janela muito mais curta (três dias): a diferença entre esta dissertação e aquele estudo está no horizonte (doze meses, compatível com a periodicidade anual da divulgação, contra três dias, adequado a um teste de reação imediata), não na família de medida.

\citeonline{barberlyon1997} documentam que o retorno ajustado ao mercado tende a apresentar menor poder estatístico e maior viés de especificação do que abordagens baseadas em carteira de referência ou empresa de controle pareada por tamanho e \textit{book-to-market}, especialmente em horizontes mais longos. A escolha por um retorno ajustado ao mercado nesta dissertação é, portanto, uma simplificação deliberada e reconhecida como tal, adequada à etapa exploratória do trabalho, mas não a especificação definitiva mais rigorosa disponível na literatura. A versão final deve considerar um modelo de mercado (CAPM) ou de fatores (Fama-French/Carhart), ou uma carteira de referência pareada por tamanho, para o cálculo definitivo do retorno anormal.
...
O modelo empírico foi construído em etapas de rigor crescente, cada uma motivada por uma ameaça específica à identificação, em um processo deliberado de teste e correção de especificação, descrito a seguir.
...
Como análise complementar, testa-se se a mudança textual também se associa à revisão do consenso de previsões de EPS dos analistas após a divulgação, usando dados de consenso do Bloomberg.

A variável de revisão é construída, para cada par (empresa, exercício $t-1$, exercício $t$) com similaridade textual computável, como a diferença percentual entre o último valor de consenso de EPS disponível antes da data de divulgação da DFP e o primeiro valor disponível depois dela, dentro de uma janela de 140 dias em torno da divulgação. A mesma progressão de rigor descrita na Seção~\ref{sec:modelo} para H1 (correlação simples; regressão agrupada por empresa, sem e com controles; e efeitos fixos de empresa e ano, quando o desenho do painel permite) é aplicada a essa variável, repetida para cada fonte de texto e frequência já usada em H1.
...
A amostra compreende 111 empresas não financeiras listadas na B3: as 66 constituintes atuais do Ibovespa (não financeiras) mais 46 empresas que integraram o índice em algum momento entre 2015 e 2024 mas não integram mais, reconstruídas a partir de dados históricos de composição do índice obtidos via Bloomberg, especificamente para corrigir o viés de sobrevivência que uma amostra baseada apenas na composição atual do índice teria. O período de análise é 2015--2024, com dados anuais (DFP) e, em caráter exploratório, dados trimestrais (ITR) para 2015--2024.
...
Para o Relatório da Administração e a seção de Fatores de Risco, o problema é o inverso: uma vez que o arquivamento é obtido, a seção relevante já vem isolada como um anexo PDF dedicado dentro do pacote de envio da CVM (padrão confirmado por leitura de exemplos reais de empresas distintas antes de ser adotado como regra geral, e não assumido a priori), de modo que não há ambiguidade de localização. O fator limitante passa a ser a \textit{cobertura da aquisição}: o Relatório da Administração foi encontrado em 71,2\% dos arquivamentos possíveis (707 de 993), reprocessando pacotes de dados já baixados, sem necessidade de novos downloads (a busca inicial verificava apenas as duas primeiras páginas de cada anexo candidato; ampliá-la para 15 páginas recuperou mais 40 arquivamentos em que o relatório aparece depois de uma capa ou índice); os Fatores de Risco, adquiridos por um pipeline novo que trata tanto o formato moderno quanto um formato legado pré-2021, atingiram 94,4\% de cobertura (923 de 978), consistente ao longo dos dez exercícios do período.
...
A Figura~\ref{fig:similaridade} compara a distribuição da similaridade de cosseno entre as cinco fontes de texto. Um padrão salta aos olhos e é relevante para a leitura dos resultados do Capítulo~\ref{ch:resultados}: as fontes de documento inteiro ou pré-isolado por seção fixa (conjunto completo de notas, Fatores de Risco) concentram-se muito mais perto de 1 (mediana de 0,95 em ambas) do que a nota de dívida isolada (mediana de 0,68 na base anual). Ou seja, o recorte estreito da nota específica de dívida captura consistentemente mais mudança textual ano a ano do que o documento inteiro ou uma seção-padrão como Fatores de Risco, nos quais boilerplate estrutural dilui a mudança relativa.
...
Antes dos testes confirmatórios de H1 e H2 (Seções~\ref{sec:h1_resultados}--\ref{sec:h2_resultados}), uma fase exploratória extensa e iterativa (cinco rodadas de teste e correção, ao longo da construção do pipeline) verificou se a similaridade TF-IDF aplicada à nota de dívida extraída automaticamente captura de fato conteúdo textual real, e não apenas ruído de extração. Essa validação não é, em si, um teste de hipótese; é o alicerce que dá suporte à leitura dos resultados nulos das seções seguintes como uma resposta genuína sobre previsibilidade de retorno, e não como um artefato de um pipeline de dados malfeito.
...
\begin{figure}[htbp]
    ...
    {\footnotesize Fonte: elaborada pelo autor a partir de \textit{similarity\_results.csv} (POC, corpus anual atual, 66 empresas).}
\end{figure}

\textbf{Dois casos concretos, lidos linha a linha.} Um exemplo de similaridade genuinamente alta: WEG 2015$\to$2016 registra 0,9915, e a comparação direta do texto mostra que a estrutura da nota (cláusulas de covenant do BNDES/FINEP, categorias de taxa fixa e indexadores TJLP/UFIR) é idêntica entre os dois anos; apenas os valores de saldo e a data de referência mudam, além de uma inversão cosmética na ordem das palavras do título. É exatamente o padrão de ``boilerplate atualizado com novos números'' que a literatura de mudança textual pressupõe. Já o caso da Yduqs ilustra o oposto: quatro pares ano a ano diferentes (2015 a 2019) registram similaridade exatamente 1,0000, com texto idêntico de 648 caracteres em todos eles. A leitura do texto revela que não é a nota de dívida real, e sim um parágrafo genérico de política contábil (reconhecimento inicial de empréstimos a valor justo) que o localizador capturou por engano nesses anos específicos, quando a nota verdadeira (numerada separadamente pela empresa) ainda não estava estilizada de forma distinguível. Todo escore degenerado (exatamente 0 ou 1) inspecionado neste projeto rastreou até uma falha de extração conhecida e explicável, não até uma falha da própria medida de similaridade.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrrrr@{}}
        ...
        \midrule
        Nota de dívida (anual)       & 671   & 0,645 & 0,715 & 0,779 & 0,776 \\
        Nota de dívida (trimestral)  & 2.138 & 0,876 & 0,900 & 0,897 & 0,671 \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrr@{}}
        ...
        \midrule
        Nota de dívida (anual)      & 0,761 & 0,876 & 0,745 \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}llrrrrl@{}}
        ...
        \midrule
        Nota de dívida (anual)      & 12m & 0,776 & 0,913 & 0,744 & 0,024 & FE: 0,913 \\
                                     & 18m & 0,800 & 0,844 & 0,721 & 0,001 & FE: 0,844 \\
        Conjunto completo de notas  & 12m & 0,660 & 0,220 & 0,427 & 0,768 & RE: 0,427 \\
                                     & 18m & 0,182 & 0,080 & 0,177 & 0,802 & RE: 0,177 \\
        Relatório da Administração  & 12m & 0,478 & 0,785 & \textbf{0,045} & 0,000 & FE: 0,785 \\
                                     & 18m & 0,683 & 0,358 & 0,862 & 0,026 & FE: 0,358 \\
        Fatores de Risco            & 12m & 0,758 & 0,567 & 0,355 & 0,160 & RE: 0,355 \\
                                     & 18m & 0,426 & 0,232 & 0,071 & 0,013 & FE: 0,232 \\
        \bottomrule
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Fonte: elaborada pelo autor. Valores são p-valores do coeficiente de similaridade, exceto a coluna Hausman (p-valor do teste de especificação) e a última coluna, que reporta o estimador escolhido pela regra de \citeonline{santoscoelho2018} (Hausman rejeita a 5\% $\rightarrow$ efeitos fixos; caso contrário, efeitos aleatórios) e o p-valor correspondente.}
\end{table}

O caso do Relatório da Administração aos 12 meses ilustra por que a regra de seleção importa: sob efeitos aleatórios sozinhos, o coeficiente pareceria significativo ($p=0{,}045$), mas o teste de Hausman rejeita decisivamente a equivalência entre os dois estimadores ($p<0{,}001$) e recomenda efeitos fixos, sob o qual o resultado é nulo ($p=0{,}785$). Aplicando a própria regra de decisão de \citeonline{santoscoelho2018} de forma correta, portanto, nenhuma fonte de texto e nenhuma janela mostra significância em nenhuma das duas versões do desenho.

Esse exercício responde diretamente à limitação que os autores apontam em seu próprio estudo: um painel mais longo, dizem eles, poderia revelar diferenças relevantes entre as abordagens de efeito fixo e aleatório que um painel curto não capta com confiança. Um painel mais de três vezes mais longo (dez exercícios contra três) permite exatamente esse teste com mais poder estatístico, e o resultado, em vez de fortalecer ou reverter o achado nulo, o confirma sob mais uma combinação de modelo e amostra. Isso não invalida a contribuição de \citeonline{santoscoelho2018}, que trata de uma pergunta diferente (valor de mercado corrente, não retorno futuro) com um recorte amostral diferente; mas sugere que, ao menos para a relação entre mudança textual e retorno futuro, a inconclusividade não é um artefato de painel curto. Ela persiste mesmo quando essa limitação específica é removida.

\section{Análise complementar (H2)}
\label{sec:h2_resultados}

Como análise complementar (Seção~\ref{sec:h2_metodologia}), testou-se se a mudança textual também se associa à revisão do consenso de EPS dos analistas em torno da data de divulgação, seguindo a mesma progressão de rigor da Seção~\ref{sec:h1_resultados} para as quatro fontes de texto, totalizando 56 testes. O resultado é um nulo limpo: nenhum dos 24 testes centrais atinge significância a 5\% (Tabela~\ref{tab:h2}).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrrrr@{}}
        ...
        \textbf{Fonte de texto} & \textbf{n} & \textbf{Simples} & \textbf{Agrup., s/c} & \textbf{Agrup., c/c} & \textbf{Ef. fixos} \\
        \midrule
        Nota de dívida (anual)       & 518   & 0,456 & 0,503 & 0,474 & 0,766 \\
        Nota de dívida (trimestral)  & 1.678 & 0,833 & 0,851 & 0,808 & 0,886 \\
...
Dos 56 testes totais (incluindo comparações de tercil e a checagem de deslistamento aplicada à revisão), 4 registraram $p<0{,}05$, um número muito próximo dos 2,8 esperados por acaso puro. Cada um dos quatro foi inspecionado individualmente: três eram comparações de tercil com padrão não monótono, e o quarto era a checagem de deslistamento no Relatório da Administração, que voltou a não ser significativa assim que testada com agrupamento por empresa ($p=0{,}050$ para $p=0{,}851$), uma ocorrência adicional, dentro deste mesmo projeto, do mesmo padrão identificado na Seção~\ref{sec:robustez}.
