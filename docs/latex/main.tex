...
\begin{itemize}
    ...
    \item Comparar o resultado obtido no recorte estreito e pouco explorado da nota de dívida com o resultado das mesmas hipóteses aplicadas a áreas de divulgação textual mais amplas e já mais exploradas na literatura internacional (conjunto completo de notas, Relatório da Administração, Fatores de Risco).
\end{itemize}
...
Nem todo caso classificado como ``não localizado'' representa necessariamente uma falha de extração: é possível que, para uma empresa específica num exercício específico, não exista de fato uma dívida relevante a divulgar. Uma primeira checagem, buscando no texto integral de cada arquivamento alguma declaração explícita do tipo ``a Companhia não possui empréstimos, financiamentos ou debêntures'', não encontrou nenhuma ocorrência nos 66 casos não localizados da base anual. Uma segunda checagem, mais precisa, foi construída diretamente da conta contábil estruturada da própria CVM para ``Empréstimos e Financiamentos'' (códigos 2.01.04, circulante, e 2.02.01, não circulante, do Balanço Patrimonial Passivo, que já incluem debêntures como subconta da mesma linha) -- um sinal item a item, não um proxy agregado como a alavancagem geral usada em uma primeira tentativa. O resultado: em 45 dos 66 casos (68\%), a empresa reporta nesse exercício um saldo de Empréstimos e Financiamentos materialmente positivo, evidência direta de que a nota provavelmente existe e a falha é de extração, não de conteúdo. Em 8 casos (12\%) o saldo é exatamente zero, a evidência mais forte disponível de ausência genuína de dívida; mais 3 casos (5\%) têm saldo positivo mas abaixo de 1\% do ativo total, provavelmente pequeno demais para justificar uma nota dedicada. Os 10 casos restantes (15\%) têm extração de texto quase vazia -- a heurística de localização não encontrou texto algum a examinar, não apenas a nota -- e não puderam ser avaliados por essa via. Essa verificação mais precisa contradiz, em pelo menos um caso concreto, a inferência obtida da alavancagem agregada: uma empresa antes classificada como caso plausível de ausência genuína, por ter alavancagem baixa, reporta pela conta específica de dívida um saldo de debêntures não circulantes de R\$~223~milhões no mesmo exercício -- um lembrete de que um proxy agregado pode induzir a erro mesmo quando aponta na direção certa em média, e a razão para preferir, sempre que disponível, a conta contábil mais específica.
...
A nota de dívida isolada em base anual é o objeto central desta dissertação (H1 e H2, Seção~\ref{sec:hipoteses}); as outras quatro fontes de texto entram apenas como critério de comparação metodológica (Seção~\ref{sec:medida}), não como parte do teste das hipóteses. A Figura~\ref{fig:similaridade} compara a distribuição da similaridade de cosseno entre as cinco fontes. Um padrão salta aos olhos e é relevante para a leitura dos resultados do Capítulo~\ref{ch:resultados}: as fontes de documento inteiro ou pré-isolado por seção fixa (conjunto completo de notas, Fatores de Risco) concentram-se muito mais perto de 1 (mediana de 0,95 em ambas) do que a nota de dívida isolada (mediana de 0,68 na base anual). Ou seja, o recorte estreito da nota específica de dívida captura consistentemente mais mudança textual ano a ano do que o documento inteiro ou uma seção-padrão como Fatores de Risco, nos quais boilerplate estrutural dilui a mudança relativa.
