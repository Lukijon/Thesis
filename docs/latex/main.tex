...
\begin{table}[htbp]
    ...
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Fonte: elaborada pelo autor a partir de dados da CVM. Nota de dívida isolada restrita a pares com extração confiável (\textit{font\_heading}) nos dois anos, Seção~\ref{sec:confiabilidade}.}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrrrr@{}}
        ...
        \midrule
        Nota de dívida (anual)       & 432   & 0,155 & 0,274 & 0,321 & 0,203 \\
        Nota de dívida (trimestral)  & 1.226 & 0,500 & 0,605 & 0,782 & 0,348 \\
    ...
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Fonte: elaborada pelo autor. Valores são p-valores; ``Agrup., s/c'' = agrupado por empresa, sem controles; ``Agrup., c/c'' = agrupado com controles. Nota de dívida (anual e trimestral): amostra restrita a pares com extração confiável (Seção~\ref{sec:confiabilidade}).}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrr@{}}
        ...
        \midrule
        Nota de dívida (anual)      & 0,649 & 0,851 & 0,860 \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}llrrrrl@{}}
        ...
        \midrule
        Nota de dívida (anual)      & 12m & 0,203 & 0,349 & 0,270 & 0,000 & FE: 0,349 \\
                                     & 18m & 0,385 & 0,506 & 0,525 & 0,001 & FE: 0,506 \\
...
Como análise complementar (Seção~\ref{sec:h2_metodologia}), testou-se se a mudança textual também se associa à revisão do consenso de EPS dos analistas em torno da data de divulgação, seguindo a mesma progressão de rigor da Seção~\ref{sec:h1_resultados} para as quatro fontes de texto (nota de dívida anual e trimestral já restritas à amostra de extração confiável, Seção~\ref{sec:confiabilidade}), totalizando 32 testes. O resultado central é um nulo limpo: nenhum dos 12 testes centrais (correlação e regressão agrupada, com e sem controles) atinge significância a 5\% (Tabela~\ref{tab:h2}).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrrrr@{}}
        ...
        \textbf{Fonte de texto} & \textbf{n} & \textbf{Simples} & \textbf{Agrup., s/c} & \textbf{Agrup., c/c} & \textbf{Ef. fixos} \\
        \midrule
        Nota de dívida (anual)       & 339   & 0,713 & 0,647 & 0,480 & 0,851 \\
        Nota de dívida (trimestral)  & 982   & 0,233 & 0,240 & 0,383 & 0,305 \\
        Conjunto completo de notas   & 316   & 0,662 & 0,569 & 0,855 & 0,866 \\
        Relatório da Administração   & 386   & 0,747 & 0,689 & 0,693 & 0,967 \\
        \bottomrule
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Nota de dívida (anual e trimestral): amostra restrita a pares com extração confiável (Seção~\ref{sec:confiabilidade}).}
\end{table}

Dos 32 testes totais (incluindo comparações de tercil e a checagem de deslistamento aplicada à revisão), 3 registraram $p<0{,}05$, um número próximo dos 1,6 esperados por acaso puro. Os três foram inspecionados individualmente: dois eram comparações de tercil com padrão não monótono (nota de dívida trimestral, $p<0{,}001$, terço médio de revisão muito acima dos dois extremos; conjunto completo de notas, $p=0{,}024$, recuo no terço médio antes de subir no terço alto -- robustos à remoção de valores extremos, portanto não um artefato isolado, mas ainda sem uma leitura monotônica compatível com H2), e o terceiro era a checagem de deslistamento no Relatório da Administração, que voltou a não ser significativa assim que testada com agrupamento por empresa ($p=0{,}050$ para $p=0{,}851$), uma ocorrência adicional, dentro deste mesmo projeto, do mesmo padrão identificado na Seção~\ref{sec:robustez}.
