...
\begin{table}[htbp]
    ...
    \end{tabular}
    \\[0.3cm]
    {\footnotesize Fonte: elaborada pelo autor. Valores são p-valores do coeficiente de TextChange, erros-padrão agrupados por empresa em todos os modelos (M0-M3 incluem efeitos fixos de empresa e ano). *M2 é a especificação principal (Seção~\ref{sec:controles}), em destaque na coluna; a linha em destaque (nota de dívida anual) é a fonte de texto principal desta dissertação (Tabela~\ref{tab:sample}). Nota de dívida (anual e trimestral): amostra restrita a pares com extração confiável (Seção~\ref{sec:confiabilidade}). Coeficientes, erros-padrão e intervalos de confiança completos para todas as células estão na Tabela~\ref{tab:h1_m2_detalhe} e em \textit{m0\_m3\_all\_sources\_results.csv}.}
\end{table}
