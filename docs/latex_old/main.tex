...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}llrr@{}}
        ...
        \rowcolor{fontechave}
        \textbf{Nota de dívida isolada (confiável)}  & \textbf{Anual} & \textbf{527} & \textbf{99} \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lrrrrr@{}}
        ...
        \midrule
        \rowcolor{fontechave}
        \textbf{Nota de dívida (anual)}       & \textbf{418} & \textbf{0,211} & \textbf{0,265} & \cellcolor{m2intersecao}\textbf{0,268} & \textbf{0,421} \\
        Nota de dívida (trimestral)  & 437 & 0,563 & 0,506 & \cellcolor{m2principal}0,286 & 0,390 \\
        Conjunto completo de notas   & 404 & 0,424 & 0,683 & \cellcolor{m2principal}0,631 & 0,611 \\
        Relatório da Administração   & 462 & 0,781 & 0,908 & \cellcolor{m2principal}0,928 & 0,949 \\
        Fatores de Risco (FRE)       & 650 & 0,420 & 0,355 & \cellcolor{m2principal}0,359 & 0,197 \\
        \bottomrule
    \end{tabular}
    \\[0.3cm]
...
Nenhuma fonte de texto mostra associação significativa com o retorno anormal em nenhum estágio da grade. É um resultado nulo consistente, não sensível à especificação de controles escolhida. A Tabela~\ref{tab:h1_m2_detalhe} detalha o coeficiente, o erro-padrão e o intervalo de confiança de 95\% de cada modelo e cada fonte.
...
O resultado nulo de H1 admite mais de uma explicação, e esta dissertação não escolhe uma única entre elas sem evidência adicional. Além da discricionariedade textual discutida acima, ao menos quatro leituras alternativas permanecem em aberto: (i) o mercado brasileiro pode incorporar a informação da nota de dívida rapidamente, antes da janela de doze meses medida aqui, tornando a previsibilidade de longo prazo testada por este desenho a margem errada para detectar o efeito; (ii) a similaridade de cosseno sobre TF-IDF capta mudança lexical, não necessariamente novidade econômica -- duas notas podem diferir muito no texto sem diferir na substância, ou o inverso; (iii) informação relevante sobre a dívida da empresa pode já estar refletida em outras divulgações que chegam ao mercado antes da nota (fatos relevantes, teleconferências, releases de resultado), esvaziando o conteúdo incremental que a nota poderia carregar; e (iv) o efeito verdadeiro pode ser pequeno e o poder estatístico desta amostra insuficiente para distingui-lo de zero. Essas leituras, e a comparação entre elas, são retomadas com mais profundidade no Capítulo~\ref{ch:conclusao}.
...
\textbf{Correção para múltiplos testes.} A família confirmatória desta dissertação -- os nove testes da especificação principal (M2), cinco de H1 (Tabela~\ref{tab:h1}) e quatro de H2 (Tabela~\ref{tab:h2}) -- foi submetida à correção de Benjamini-Hochberg \cite{benjaminihochberg1995} (taxa de descoberta falsa, $q=0{,}05$). O menor p-valor bruto da família, $0{,}268$ (nota de dívida anual, H1), passa a $0{,}808$ após o ajuste; nenhum dos nove testes sobrevive ao corte de 5\%. M0, M1 e M3 não entram nessa família por serem variantes cumulativas de robustez do mesmo coeficiente por fonte, não hipóteses independentes; os testes exploratórios e de robustez (deslistamento, comparação com o desenho de Santos e Coelho, portfólio calendário, bateria secundária de H2) também ficam de fora -- já são tratados e rotulados como exploratórios no texto, a resposta apropriada à sua multiplicidade, não uma correção formal reservada à família confirmatória pré-especificada.
