...
- [x] **Consenso de previsões de EPS dos analistas (Bloomberg BEst)** — ✅ recebido, cobertura conferida, H2 testado em todos os cenários (2026-08-29)

  Você adicionou `data/interim/eps estimates.csv` (export do Bloomberg, `is_eps` com `fpt=Q`/`ae=E` — série histórica trimestral do consenso de EPS, uma linha por data de referência, confirmada como o histórico de estimativas por trimestre); movido para `data/raw/analysts/eps_consensus_bloomberg.csv` (git-ignorado, como todo dado Bloomberg). 153 tickers (mesmo conjunto do arquivo de preços), 133 datas trimestrais, 6/2013–12/2026. Conferido contra o universo antes de aceitar: **64 das 66 empresas atuais do Ibovespa têm pelo menos alguma cobertura** (58 com cobertura densa, ≥20 dos ~52 trimestres desde 2015); **Klabin (KLBN11) e Iguatemi (IGTI11) têm cobertura zero**. Cruzamento contra as 45 empresas históricas/deslistadas agora feito: **27 têm cobertura** (das 30 que resolvem para um ticker vivo). Revisão definida (`src/analysis/compute_eps_revisions.py`) como a variação do consenso *rolling* (não do mesmo trimestre-alvo — ver caveat abaixo) entre o snapshot anterior e o posterior à data de divulgação, janela de 140 dias. Bateria completa rodada contra os 4 cenários de similaridade (nota estreita anual/trimestral, documento inteiro, relatório da administração) — **nulo em todos os testes centrais**, ver `reports/h2_eps_revision_exploration.md`. Caveat real e ainda não resolvido: a medida de revisão "mesmo trimestre-alvo" mencionada abaixo não é construível a partir deste export (é uma série rolling, uma coluna por ticker) — precisaria de um novo pull do Bloomberg (uma coluna por trimestre-alvo fixo) se quiser essa versão mais precisa.
